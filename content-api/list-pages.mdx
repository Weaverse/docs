...
| `data[].handle` | string | Page handle (e.g., product handle, `default`, `pages/gift-shop`). Use this exact value when calling [Get Page](/content-api/get-page); custom page handles may contain `/`. |
| `data[].locale` | string | Locale code |
| `data[].pageId` | string | Associated page ID |
| `nextCursor` | string \| null | Cursor for the next page |

<Tip>
  Treat `type` + `handle` as an opaque pair. For example, a row with `type: "CUSTOM"` and `handle: "pages/gift-shop"` is fetched from `/projects/:projectId/pages/CUSTOM/pages/gift-shop`.
</Tip>
