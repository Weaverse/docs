...
| `handle` | string | Yes | Page handle (e.g., `default`, `blue-shirt`, `pages/gift-shop`). For `CUSTOM` pages, use the exact handle returned by [List Pages](/content-api/list-pages); it may contain `/` and is captured as the rest of the path. |
...
| `includeDefaults` | boolean | `false` | When `true`, fills absent component setting keys with `defaultValue`s from the registered schemas. Stored values always win. |
...
<Tip>
  Use `includeDefaults=true` when an external renderer needs a complete component configuration. Leave it off when you need to distinguish explicitly stored values from schema defaults.
</Tip>
...
<Warning>
  **Always pass `locale` for localized projects.** Page content is stored per
  ...
  `…/pages/PRODUCT/default` *without* `?locale` requests the **base locale**
  first. On a localized project, that may resolve to a different assignment
  through the fallback sequence below — and can be empty or unedited. This is
  the most common reason a published change appears in the storefront/editor
  but seems "missing" from the API.
...
### Locale and template fallback

The API evaluates this sequence at each project level:

1. The requested handle's market match, when the project uses market-first resolution
2. The requested handle with the exact locale
3. The requested handle with the empty locale (`""`)
4. The requested handle with the legacy `en-us` locale
5. For a non-`CUSTOM` request with a non-empty handle, the empty-handle template in the same market, exact-locale, empty-locale, then legacy `en-us` order

If the current project has no match, the same sequence runs for its parent and then grandparent project. The resolver does not consult a configured project default locale. If no assignment is found after the parent cascade, the API returns `404`.
...
<Tabs>
  <Tab title="cURL">
    ...
    # Get a custom page whose handle includes a slash
    curl -H "Authorization: Bearer YOUR_API_KEY" \
      "https://studio.weaverse.io/api/v1/content/projects/clx1abc23def456ghij/pages/CUSTOM/pages/gift-shop"
    ```
  </Tab>
  <Tab title="TypeScript">
    ...
    // Get a custom page; pass the exact handle from List Pages
    const giftShop = await getPage('clx1abc23def456ghij', 'CUSTOM', 'pages/gift-shop')
    console.log(productPage.handle, giftShop.handle)
  ...
  <Tab title="Python">
    ...
    # Get a custom page; pass the exact handle from List Pages
    gift_shop = get_page("clx1abc23def456ghij", "CUSTOM", "pages/gift-shop")
    ```
  </Tab>
</Tabs>
