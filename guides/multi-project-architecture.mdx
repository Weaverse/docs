...
// `hydrogenContext` and `request` are the values the factory received. On
// Oxygen the runtime env arrives with the request — there is no `process.env`.
export function createWeaverseClient({
  hydrogenContext,
  request,
  /* ...rest */
}: CreateWeaverseClientArgs) {
  return new WeaverseClient({
    ...
    projectId: () => {
      ...
      return hydrogenContext.env.WEAVERSE_PROJECT_ID;
    },
  });
}
```
...
import type { HydrogenThemeSchema, HydrogenComponent } from "@weaverse/hydrogen";
...
export function createWeaverseClient(args: {
  ...
  return new WeaverseClient({
    ...
    projectId: () => {
      ...
      // Map domains to project IDs — runtime env comes from the Hydrogen context
      const projectMap: Record<string, string> = {
        "https://mystore.se": hydrogenContext.env.WEAVERSE_PROJECT_SWEDEN!,
        "https://mystore.fr": hydrogenContext.env.WEAVERSE_PROJECT_FRANCE!,
        "https://mystore.de": hydrogenContext.env.WEAVERSE_PROJECT_GERMANY!,
      };

      return projectMap[origin] || hydrogenContext.env.WEAVERSE_PROJECT_ID!;
    },
  });
}
```
...
<Note>
On Oxygen the storefront runs in a worker, so there is no `process.env`. Read runtime variables from `hydrogenContext.env` inside your factory, or from `context.env` inside a route loader.
</Note>
...
export default {
  async fetch(request: Request, env: Env, context: ExecutionContext) {
    ...
    // Preserve the RouterContextProvider instance React Router expects.
    Object.assign(hydrogenContext, { weaverse });
    return handleRequest(request, hydrogenContext);
  },
};
```
...
```typescript
// app/routes/home.tsx
// Registered inside the :locale? prefix with index("routes/home.tsx")
...
export function createWeaverseClient(args: CreateWeaverseClientArgs) {
  ...
  return new WeaverseClient({
    ...
    projectId: () => {
      ...
      let { env } = hydrogenContext;

      // Market-specific projects
      if (origin === "https://mystore.se") {
        return env.WEAVERSE_PROJECT_SWEDEN!;
      }

      if (origin === "https://mystore.fr") {
        return env.WEAVERSE_PROJECT_FRANCE!;
      }

      if (origin === "https://mystore.de") {
        return env.WEAVERSE_PROJECT_GERMANY!;
      }
      ...
      return env.WEAVERSE_PROJECT_DEFAULT!;
    },
  });
}

// app/routes/home.tsx (registered with index("routes/home.tsx"))
export async function loader({ context }: LoaderFunctionArgs) {
  let { weaverse } = context;
    ...
    experiments: [
      {
        id: "green-theme-test",
        variants: [
          { id: "control", projectId: hydrogenContext.env.WEAVERSE_PROJECT_CONTROL! },
          { id: "b", projectId: hydrogenContext.env.WEAVERSE_PROJECT_VARIANT_B! },
        ],
      },
    ],
  });
...
Variant `weight` is relative — omit it for an even split. For an 80/20 test, weight the variant down (only the ratio matters). This runs in the same factory as above, so `hydrogenContext.env` is still the source of the runtime variables:
  ...
  experiments: [
    {
      id: "green-theme-test",
      variants: [
        { id: "control", projectId: hydrogenContext.env.WEAVERSE_PROJECT_CONTROL! }, // weight 1
        { id: "b", weight: 0.25, projectId: hydrogenContext.env.WEAVERSE_PROJECT_VARIANT_B! }, // ~20%
      ],
    },
  ],
});
...
// Wrapping `<Outlet />` is required: a self-closing `<WeaverseExperiments />`
    ...
    experiments: [
      {
        id: "pdp-layout-test",
        variants: [
          { id: "control", projectId: env.WEAVERSE_PROJECT_CONTROL! },
          { id: "b", projectId: env.WEAVERSE_PROJECT_PRODUCT_VARIANT_B! },
        ],
      },
    ],
  });
...
The loader returns `assignments` alongside `weaverseData`, so this route can feed the chosen variant into its own `<Analytics.Provider customData>` / `<WeaverseExperiments>` — exactly like the global setup above — making variant-segmented events and `useExperiment("pdp-layout-test")` work on this route too.
  ...
  return new WeaverseClient({
    ...
    projectId: () => {
      ...
      const brandProjects: Record<string, string> = {
        "brand-a": hydrogenContext.env.WEAVERSE_PROJECT_BRAND_A!,
        "brand-b": hydrogenContext.env.WEAVERSE_PROJECT_BRAND_B!,
      };

      return brandProjects[subdomain] || hydrogenContext.env.WEAVERSE_PROJECT_ID!;
    },
  });
}
```
...
// Registered inside prefix("products", [...]) as index("routes/products/list.tsx")
export async function loader({ request, context }: LoaderFunctionArgs) {
  let { weaverse, storefront } = context;
  let url = new URL(request.url);
  ...
  let weaverseData = await weaverse.loadPage({ type: "ALL_PRODUCTS" });
...
// The runtime env is passed in — this module has no access to `process.env`.
export function getBrandConfig(brand: Brand, env: Env) {
  const configs = {
    "brand-a": {
      name: "Premium Brand",
      projectId: env.WEAVERSE_PROJECT_BRAND_A!,
      productTag: "Premium",
      theme: "luxury",
    },
    "brand-b": {
      name: "Affordable Brand",
      projectId: env.WEAVERSE_PROJECT_BRAND_B!,
...
export function createWeaverseClient(args: CreateWeaverseClientArgs) {
  ...
  let brandConfig = getBrandConfig(brand, hydrogenContext.env);
...
```typescript
// app/routes/special-campaign.tsx
// Registered in app/routes.ts as:
// route("special-campaign/:slug", "routes/special-campaign.tsx")
export async function loader({ context }: LoaderFunctionArgs) {
  let { weaverse, env } = context;
  ...
  let weaverseData = await weaverse.loadPage({
    type: "CUSTOM",
    projectId: env.WEAVERSE_PROJECT_CAMPAIGN, // Route-level override
  });
...
export async function loader({ params, context, request }: LoaderFunctionArgs) {
  ...
  if (!productHandle) throw new Response("product", { status: 404 });
...
export async function loader({ params, context, request }: LoaderFunctionArgs) {
  ...
  if (!collectionHandle) throw new Response("collection", { status: 404 });

  // Map specific collections to different projects
  const collectionProjectMap: Record<string, string> = {
    "vip-collection": env.WEAVERSE_PROJECT_VIP!,
    "black-friday": env.WEAVERSE_PROJECT_SALE!,
    "new-arrivals": env.WEAVERSE_PROJECT_NEW!,
  };
...
Each snippet below is the `projectId` option of the `createWeaverseClient` factory shown earlier, so `request` and `hydrogenContext` (and therefore `hydrogenContext.env`) are in scope.
...
projectId: () => {
  ...
  if (path.startsWith("/pro")) {
    return env.WEAVERSE_PROJECT_PRO!;
  }

  // /basic routes use basic project
  if (path.startsWith("/basic")) {
    return env.WEAVERSE_PROJECT_BASIC!;
  }
...
projectId: () => {
  ...
  if (variant === "b") {
    return env.WEAVERSE_PROJECT_VARIANT_B!;
  }

  return env.WEAVERSE_PROJECT_CONTROL!;
}
```
...
projectId: () => {
  ...
  if (subdomain === "premium") {
    return env.WEAVERSE_PROJECT_PREMIUM!;
  }

  // Priority 3: Path-based
  if (path.startsWith("/campaign")) {
    return env.WEAVERSE_PROJECT_CAMPAIGN!;
  }
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  return data({ weaverseData }, { headers });
}
```
...
export async function loader({ context }: LoaderFunctionArgs) {
  let { weaverse } = context;
  let projectId = context.env.WEAVERSE_PROJECT_CAMPAIGN;
...
**Solutions:**
- Verify the runtime environment variable exists: `console.log(context.env.WEAVERSE_PROJECT_CAMPAIGN)`
...
Except where noted, each snippet below is the `projectId` option inside `createWeaverseClient`, where `request` and `hydrogenContext.env` are in scope.

### Dynamic Project Selection

**Based on User Attributes:**
```typescript
// Resolve asynchronous user/session data before constructing WeaverseClient.
// The projectId callback itself is synchronous.
let user = await getUser(request);
let projectId = user.tier === "premium"
  ? hydrogenContext.env.WEAVERSE_PROJECT_PREMIUM
  : hydrogenContext.env.WEAVERSE_PROJECT_STANDARD;

let weaverse = new WeaverseClient({
  ...hydrogenContext,
  request,
  projectId,
  // ...cache, themeSchema, components
});
```

**Based on Geographic Location:**
```typescript
// Oxygen forwards the visitor's country on the request
projectId: () => {
  let country = request.headers.get("oxygen-buyer-country");
  let { env } = hydrogenContext;

  const countryProjects: Record<string, string> = {
    "SE": env.WEAVERSE_PROJECT_SWEDEN!,
    "FR": env.WEAVERSE_PROJECT_FRANCE!,
    "DE": env.WEAVERSE_PROJECT_GERMANY!,
  };

  return (country ? countryProjects[country] : undefined) || env.WEAVERSE_PROJECT_ID!;
}
```
...
projectId: () => {
  ...
  if (origin === "https://mystore.jp") {
    return env.WEAVERSE_PROJECT_JAPAN!;
  }
...
projectId: () => {
  ...
  if (featureFlag === "beta") {
    return env.WEAVERSE_PROJECT_BETA!;
  }

  // Production project (stable)
  return env.WEAVERSE_PROJECT_PROD!;
}
```
...
- [Custom Routing](/features/custom-routing) - Advanced URL patterns
