...
<Note>
The storefront runs in an Oxygen worker, so there is no `process.env`. Read runtime variables from `hydrogenContext.env` in your factory, or from `context.env` inside a route loader.
</Note>
