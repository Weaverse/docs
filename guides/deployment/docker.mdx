...
Run the container with those values injected at start time:
...
<Tip>
If a tool in your chain insists on reading a `.env` file from disk, write it at container **start** from the injected environment via an entrypoint script — never at build time. The file then exists only in the running container's writable layer, not in the distributable image.
</Tip>
