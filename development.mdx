...
- **Node.js 22.12+** installed (required by the Pilot theme)
...
Generate `SESSION_SECRET` with `openssl rand -hex 32` or `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`. If you scaffold with the Weaverse CLI, it is generated for you.

For real Shopify data, stores on a Shopify plan should install the Hydrogen sales channel (https://apps.shopify.com/hydrogen), create a Hydrogen storefront project, then run `npx shopify hydrogen env pull` or copy values from **Storefront Settings → Environments and variables**. Development stores or stores without a Shopify plan should use the Headless sales channel (https://apps.shopify.com/headless) and copy the Storefront API values into `.env`.
...
- Study the [API Reference](/api-reference/introduction)
