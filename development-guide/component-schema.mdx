...
<Warning>
`settings` does **not** replace or override `inspector`. Both arrays are merged, so any group you copied from `inspector` into `settings` without deleting the original will appear twice in the Studio settings panel. Remove each `inspector` group as soon as you have migrated it.
</Warning>
