...
1. **Development Diagnostics**: Schedules asynchronous Zod validation outside production and logs issues as warnings
2. **Type Checking**: Accepts the public `SchemaType`, providing TypeScript checks and autocompletion while authoring
3. **Non-Blocking Runtime**: Returns the original schema immediately and does not throw validation or import failures through the call
4. **Production Footprint**: Skips the validation import in production builds
...
The `createSchema()` function returns the supplied schema immediately. In development, it starts asynchronous validation and logs any issues as warnings; those warnings do not stop a build or replace the schema.
...
The recommended way to define component schemas is using the `createSchema()` function, which provides `SchemaType` checking and non-blocking development diagnostics:
...
- **Does not throw validation or import failures** through `createSchema()`
- **Skips the validation path in production**
...
For a complete reference on input types and configurations, refer to the [Input Settings guide](/development-guide/input-settings).
...
```tsx
// Direct annotation (supported)
import type { HydrogenComponentSchema } from '@weaverse/hydrogen';

// Helper with development diagnostics
import { createSchema } from '@weaverse/hydrogen';
```
...
The helper keeps the same `SchemaType` contract. It returns immediately, schedules asynchronous warning-based validation only in development, and adds no validation path in production.
...
```tsx
// Transition approach — renders BOTH sets of groups
export let schema = createSchema({
  type: 'my-component',
  title: 'My Component',
  settings: [
...
<Warning>
`settings` does **not** replace or override `inspector`. The SDK concatenates both arrays before extracting defaults and registering the schema, so a group copied into `settings` without deleting the original is submitted twice to Studio. Remove each `inspector` group as soon as you have migrated it.
</Warning>
...
1. **Schema Validation Warnings**: In development, `createSchema()` may log asynchronous warnings for missing required fields, invalid input types, or malformed configuration. It still returns the original schema and does not fail the build.

2. **Duplicate Type Error**: Each component must have a unique `type`. Check for duplicates across your entire theme.

3. **Missing Required Properties**: Ensure `title` and `type` are defined. TypeScript checks these for typed schemas; the development validator can also report them as warnings.

4. **Invalid Input Types**: Verify that input `type` values match those supported by Weaverse. Development validation reports unsupported values as warnings.
...
- [Input Settings Guide](/development-guide/input-settings): Detailed information on all available input types
- [Weaverse Component Guide](/development-guide/weaverse-component): Comprehensive guide to creating components
- [Example Components](/resources/example-components): Collection of sample components for reference
