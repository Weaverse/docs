...
6. **`SHOP_ID`**: Your Shopify store ID.
...
3. **`WEAVERSE_HOST`** (optional): The host URL for Weaverse services. Default: **`https://studio.weaverse.io/dashboard`**.
4. **`WEAVERSE_API_BASE`** (optional): The base URL for Weaverse API services. Default: **`https://api.weaverse.io`**.
...
3. **`ALI_REVIEWS_API_KEY`**: API key for AliReviews integration.
...
PUBLIC_STOREFRONT_API_TOKEN="your-demo-storefront-api-token"
...
WEAVERSE_HOST="https://studio.weaverse.io/dashboard"  # Optional
WEAVERSE_API_BASE="https://api.weaverse.io"  # Optional
...
ALI_REVIEWS_API_KEY="your-ali-reviews-api-key"  # Optional
...
### Weaverse Project ID
1. Log in to [Weaverse Studio](https://studio.weaverse.io/dashboard)
...
export async function loader({ context }: LoaderFunctionArgs) {
  const { env } = context;
  return { storeDomain: env.PUBLIC_STORE_DOMAIN };
}
