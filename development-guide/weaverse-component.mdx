...
function MyComponent({ ref, title, description, children, ...rest }: MyComponentProps) {
  return (
...
export async function loader({ params, context }: LoaderFunctionArgs) {
  ...
  // Return a plain object — `json()` is no longer needed in React Router v7
  return { product };
}
...
function FeaturedProduct({ ref, loaderData, product: selectedProduct, ...rest }: FeaturedProductProps) {
  const product = loaderData?.product;
  
  if (!product) {
    return <section ref={ref} {...rest}>Select a product in the editor</section>;
  }
...
export async function loader({ weaverse }: ComponentLoaderArgs) {
  try {
    const data = await fetchData(weaverse);
    return { data };
  } catch (error) {
...
```tsx
export async function loader({ weaverse }: ComponentLoaderArgs) {
  const rawData = await fetchData(weaverse);
  return {
...
export async function loader({ weaverse }: ComponentLoaderArgs) {
  ...
  // Signature: fetchWithCache(url, options) — the cache strategy goes in `options.strategy`
  const data = await fetchWithCache(
    'https://api.example.com/data',
    {
      ...
      strategy: CacheLong(), // Use appropriate cache strategy
    }
  );

  // POST requests work the same way — the body is part of the cache key
...
const data = await weaverse.fetchWithCache(
  'https://api.example.com/data',
  { method: 'GET', strategy: cacheStrategy }
);
...
2. **Component not selectable in Studio**
   ...
   - See the [Component Not Selectable](/troubleshooting/component-not-selectable) troubleshooting guide for detailed fixes
...
- See [Example Components](/resources/example-components) for inspiration and reference implementations
- Join our [Community](/community/community) for support and sharing best practices
