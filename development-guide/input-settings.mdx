...
- [Basic Inputs](/development-guide/input-settings#basic-inputs)

- [Resource Picker Inputs](/development-guide/input-settings#resource-picker-inputs)
...
A theme can hide files from the gallery so a picker only offers what belongs in it. Declare the store-wide
default on [`HydrogenThemeSchema.media`](/api-reference/types#hydrogenthemeschema), and override it for a
single input through `configs`:
...
| Field                      | Effect                                                                                                                                              |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `excludeFilenamePrefixes`  | Hides files whose name starts with any of the listed prefixes. The wildcard is added for you, so pass `"thumb_v"` rather than `"thumb_v*"`.            |
| `excludeProductFiles`      | Hides files used in products. This matches usage, not origin: a file uploaded to the Files page and later attached to a product is hidden as well.     |

An input's `configs` overrides the theme value **field by field**:

- the key is absent — the input inherits the theme's value
- an array or boolean is present — it replaces the theme's value for this picker
- `excludeFilenamePrefixes: []` — this picker shows every file, ignoring the theme's list

Declaring one field never resets the other, and there is no flag to enable overriding: the presence of the
key is the signal.

Pickers opened outside an input schema — the SEO image field, the rich text editor and the Translation
Manager — apply the theme's rules alone. A theme that declares no `media` key filters nothing, which is not
an error.
...
We highly recommend developers use the `Image` component from the
**`@shopify/hydrogen`** package to render images. It's optimized to work with the data returned from
...
- **Filtering the gallery**: `video` inputs accept the same `configs` as `image` — see
  [Filtering the Media Manager gallery](#filtering-the-media-manager-gallery).
...
Now that you have a solid understanding of Input Settings, let's learn how to render a Weaverse page in the next article: [Rendering a Weaverse Page](/guides/rendering-page).