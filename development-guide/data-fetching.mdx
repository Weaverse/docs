...
export let loader = async ({ weaverse, data }: ComponentLoaderArgs<ProductDetailData>) => {
  ...
  const [productData, reviewsData, inventoryData] = await Promise.all([
    ...
    fetchWithCache(`https://api.reviews.example/products/${productHandle}`, {
      headers: { 'Authorization': ['Bearer', env.REVIEWS_API_KEY].join(' ') }
    }),
...
- [Component Schema](/development-guide/component-schema) - Learn how to define component schemas, including the `shouldRevalidate` property
- [Input Settings](/development-guide/input-settings) - Comprehensive guide to all input types available in Weaverse
- [Rendering a Weaverse Page](/guides/rendering-page) - Understanding how Weaverse pages are rendered
- [Project Structure](/intro/project-structure) - Learn how the project is organized
