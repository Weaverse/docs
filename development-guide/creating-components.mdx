...
# Building Weaverse Components

This comprehensive guide walks you through creating Weaverse components from scratch, covering everything from basic component structure to advanced patterns with TypeScript and React 19.
...
Weaverse components are built using modern React patterns with TypeScript. Components can be structured in different ways depending on complexity:

### Single File Components

For simple components, everything can be in one file:

```tsx
// app/sections/hero-banner.tsx
  ...
  settings: [
    {
      group: 'Content',
      inputs: [
        { type: 'text', name: 'title', label: 'Title', defaultValue: 'Welcome to Our Store' },
        { type: 'textarea', name: 'subtitle', label: 'Subtitle', defaultValue: 'Discover amazing products' }
      ]
    },
    {
      group: 'Style',
      inputs: [
        { type: 'color', name: 'backgroundColor', label: 'Background Color', defaultValue: '#f8f9fa' }
      ]
    }
  ]
})
...
For complex components, separate files for better organization:

```
app/sections/product-grid/
├── index.tsx           # Main component
├── schema.ts          # Schema definition
├── loader.ts          # Data loading (optional)
...
Here's a more complex component with separate schema file and data loading:
...
settings: [
  {
    ...
    inputs: [
      ...
      { type: "range", name: "opacity", label: "Opacity", configs: { min: 0, max: 100 } },
       { type: "switch", name: "enabled", label: "Enable Feature" },
      { type: "select", name: "layout", label: "Layout", configs: {
       ...
       // E-commerce inputs
       { type: "product", name: "featuredProduct", label: "Featured Product" },
       { type: "product-list", name: "products", label: "Product List" },
       { type: "collection", name: "collection", label: "Collection" },
       { type: "collection-list", name: "collections", label: "Collection List" },
       { type: "blog", name: "blog", label: "Blog" },
       // Advanced inputs
       { type: "datepicker", name: "date", label: "Date" },
       { type: "position", name: "position", label: "Position" },
       { type: "map-autocomplete", name: "location", label: "Location" },
       { type: "metaobject", name: "metaobject", label: "Metaobject" },
       // Special inputs
       { type: "toggle-group", name: "alignment", label: "Alignment", configs: {
         options: [
       ...
       { type: "swatches", name: "colors", label: "Color Swatches" },
    ],
  },
]
```
...
- Learn about [Input Settings](development-guide/input-settings)
- Master [Component Schemas](development-guide/component-schema)  
- Explore [Data Fetching](development-guide/data-fetching)
- Review [Styling & Theming](development-guide/styling-theming)

For more examples, check out the [Example Components](resources/example-components) and join our [Slack community](https://wvse.cc/weaverse-slack) for help.