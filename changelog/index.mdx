...
<Update label="2026.10.13" description="September 24, 2026" tags={["Studio"]}>
  ### Weaverse Studio

  #### Fixes

  - **Manage previews fits on screen again**: Long detected hostnames no longer push the content past the edges of the dialog, the preview lists no longer leave empty space below them, and the dialog scrolls on smaller screens.
  - **Clearer help when a local preview can't connect**: If you're developing locally or using an AI coding agent, Studio now asks you to make sure your local dev server is running. If you're not, it points you to Manage previews to switch to your live storefront domain running the Weaverse Hydrogen theme.
</Update>

<Update label="2026.10.12" description="September 23, 2026" tags={["Studio"]}>
  ### Weaverse Studio

  #### New Features

  - **Save global sections as drafts**: Save changes to a shared section without changing your live storefront, then reopen that draft from another page that uses the section.
  - **Publish a section without publishing the page**: Use **Publish section** to update a global section across the pages that use it while keeping other page edits unpublished. Page publishing leaves your global-section changes separate.
</Update>

<Update label="2026.10.8" description="September 20, 2026" tags={["Studio", "API"]}>
  ### Weaverse Studio

  #### Fixes

  - **Publishing no longer gets stuck on "Not found"**: If you published a page and then hit Publish again, Studio could keep rejecting it until you reloaded. It now recovers on its own and tells you what happened.
  - **Hidden elements no longer open another element's settings**: Selecting a hidden row in the Page Outline used to leave the settings panel showing whatever you had selected before, so edits could land on the wrong element. The panel now says the element is hidden, and elements inside a hidden parent explain which parent is hiding them.
  - **The outline tells you when the preview drops**: When the preview connection fails for good, the Sections list no longer sits on placeholder rows forever. You get a clear message and a retry button.
  - **Clearer message when the preview is blocked**: Running Studio against a local theme over an insecure address now says your browser blocked the preview and how to fix it, instead of claiming your dev server is not running.
  - **Product and collection pickers work in the demo**: Browsing resources in the demo project returned an error. It works again.

  #### Improvements

  - **Media Manager exclusions now live in theme code**: File exclusions move from a project setting to your theme, set store-wide or per image and video input, so every project on the theme behaves the same and merchants no longer need to configure it. Requires `@weaverse/hydrogen` 5.21.2 or newer.

  ### Content API

  - **Internal template pages are labelled**: The page list marks internal template markers so you can filter them out, and they can no longer be deleted by handle.
</Update>

<Update label="2026.10.6" description="September 18, 2026" tags={["Studio", "SDK", "Theme"]}>
  ### Weaverse Studio

  #### New Features

  - **Agent Skills for your coding assistant**: The Deployment Guide is now an Agent Skills catalog. Install the skill pack once, pick the task you want — project setup, theme updates, Figma to Weaverse, analytics, markets and localization — and copy a ready-made prompt into Claude Code, Codex, Cursor or another assistant.
  - **See and control the hostnames using your project**: Manage previews now lists every hostname observed requesting your project's content, when it was first and last seen, and whether it is a saved preview. Add one as a preview, or block it, in a click.

  #### Improvements

  - **Usage read straight from Shopify**: If your usage is billed through Shopify, the figure in your dashboard is now read directly from Shopify, so it matches the usage Shopify has recorded against your current cap cycle.
  - **Translation progress you can trust**: Translation jobs now report when they fail instead of finishing quietly, and an accepted translation refreshes right away instead of showing the previous version.

  #### Bug Fixes

  - **Saved translations no longer go missing on your storefront**: Translations for fields you left empty in your default language are now served on the live storefront instead of being filtered out.
  - **Previews stay on the page you are editing**: The editor keeps the preview pointed at the right page and the right preview address through page switches, reloads and slow connections.
  - **Page history survives a repair**: Restoring an earlier version still works on pages that have been repaired.
  ...
  - **Eight more languages**: Pilot now manages every storefront string through the Weaverse Translation Manager and ships with eight additional languages. Switching language loads the matching content instead of leaving text from the previous one behind.
  - **Deploy to Cloudflare Workers**: Pilot includes the configuration and environment setup for running your storefront on Cloudflare Workers.

  #### Bug Fixes

  - **Shoppers stay in the market they chose**: Adding to cart, signing in, signing out and following a redirect all keep the shopper on the market they were browsing, so prices and currency stay consistent.
  - **A cleaner media picker**: Generated thumbnails and product media are hidden from the Media Manager by default, so you see the files you actually uploaded. The hotspots picker still shows everything.
  ...
  - **Choose what the media picker hides**: Themes can declare which generated or imported media to exclude from image and video pickers, in the theme schema. Released in `@weaverse/hydrogen` v5.21.0.

  #### Improvements

  - **Storefronts identify themselves**: A storefront now sends its own origin with theme-settings requests, which is what the new hostname list in Manage previews reads. Released in `@weaverse/hydrogen` v5.21.2.
</Update>

<Update label="2026.9.25" description="September 7, 2026" tags={["Studio", "API"]}>
  ### New Features

  - **Page translations over the Content API**: Read and write a page's translations programmatically, so your own tools and AI assistants can localize content without opening Studio. See the [Content API overview](/content-api/overview).

  ### Improvements

  - **A way forward from an empty Scheduled publishing list**: The Scheduled publishing manager now explains how to schedule a page instead of showing an empty screen.
  - **Media Manager search and sort refresh properly**: Changing a search term or sort order reloads the gallery instead of leaving the previous results on screen.
  - **Safer resets and version history**: Resetting a localized page is recoverable, and version history no longer breaks on an incomplete saved version.

  ### Bug Fixes

  - **Create pages in a market that has none yet**: Adding a page to a secondary market with no existing pages now works.
  - **Edits stay with the page you selected**: Renaming or editing a custom page applies to the page in view, and a pending rename survives switching away and back.
  - **Imports keep their language and market**: Imported pages keep the locale and market they were exported from.
</Update>

<Update label="2026.9.1" description="September 1, 2026" tags={["Studio"]}>
  ### New Features

  - **Save a draft of a page that is already published**: Choose **Save draft** from the Publish button to store your work on a live page. The published page stays exactly as it is until you press Publish.
  - **Pick up a teammate's draft**: Opening a page that has a saved draft resumes it automatically, so two people can work on the same live page without one overwriting the other. Your unsaved edits are never replaced by an incoming draft.

  ### Improvements

  - **Project and page changes save together**: A save now applies to the project and the page as one operation, so a partial save can no longer leave the two out of step.

  ### Bug Fixes

  - **Page selection recovers after a reload**: The editor returns to the page you were on instead of losing your selection.
</Update>

<Update label="2026.8.11" description="August 11, 2026" tags={["Studio"]}>
  ### Improvements

  - **More reliable page publishing**: Saving and publishing now keep your page content in sync, including when several updates happen close together.
  - **Safer page history and scheduling**: Restoring or scheduling a saved version now keeps it matched to the correct page.

  ### Bug Fixes

  - **Clearer custom page name conflicts**: If a custom page name is already in use, Weaverse now tells you clearly so you can choose another one.
</Update>

<Update label="2026.7.18" description="July 7, 2026" tags={["Studio"]}>
  ### Improvements

  - **A clearer path when a preview won't load**: When your storefront preview can't connect or has nothing to edit yet, the preview screen now points you straight to the fix — open the setup guide, switch to a working preview, or manage your preview URLs — instead of leaving you at a dead end. The preview, setup, and Manage previews screens now share a consistent look and work together smoothly.

  ### Bug Fixes

  - **Setup guide fits on smaller screens**: The "How to set up project" guide no longer runs off the edge of shorter windows — the title and close button stay visible while the steps scroll.
</Update>

<Update label="2026.7.17" description="July 7, 2026" tags={["API"]}>
  ### New Features

  - **Get components with their defaults filled in**: When you fetch a single page from the Content API, you can now ask for every component to come back with its default values already included. Add `?includeDefaults=true` to your request and any setting you left untouched arrives with its built-in default — no need to look it up yourself.
</Update>

<Update label="2026.7.16" description="July 3, 2026" tags={["API"]}>
  ### New Features

  - **Write to your content over the API**: The Content API can now make changes, not just read. Update your theme settings and project details programmatically, so your own tools and AI assistants can push updates back into Weaverse. See the [Content API overview](/content-api/overview).
</Update>

<Update label="2026.7.11" description="July 1, 2026" tags={["Studio"]}>
  ### New Features

  - **Keyboard shortcuts guide**: A new quick-reference lists every editor keyboard shortcut, so you can move around the editor without reaching for the mouse.

  ### Improvements

  - **Clearer error when a save fails**: If a save doesn't go through, the editor now tells you the actual reason instead of a generic message, so it's faster to fix and try again.
</Update>

<Update label="2026.7.8" description="June 30, 2026" tags={["Studio"]}>
  ### Bug Fixes

  - **Setup guide fits on smaller screens**: The "How to set up project" guide no longer runs off the edge of shorter windows, and now links straight to the full guide.
  - **Tidier blog article details on mobile**: The author and date on blog articles now line up correctly on small screens.
</Update>

<Update label="2026.7.2" description="June 24, 2026" tags={["Studio"]}>
  ### New Features

  - **Connect a local AI coding assistant**: You can now link a local AI coding assistant to your project so it can make edits and preview them against your own development server.
  - **Jump back to setup when a preview won't load**: If your local preview can't connect, the preview screen now gives you a direct link back into the setup guide to get things running again.
</Update>

<Update label="2026.6.29" description="June 23, 2026" tags={["Studio", "Marketing"]}>
  ### New Features

  - **Project backups**: Save and restore snapshots of your project's content. Each backup shows you a plain-language summary of what changed, so you can see exactly what a restore would alter before you go ahead.
  - **New Careers and Partners pages**: Added a Careers page and refreshed the Partners page on our website.

  ### Improvements

  - **Fewer stale resource pickers**: Picking a product, collection, or other resource is more reliable, with fewer cases of a picker showing outdated options or losing your selection while editing.
</Update>

<Update label="2026.6.27" description="June 21, 2026" tags={["Studio"]}>
  ### New Features

  - **Shopify Sidekick can read your Weaverse content**: New tools let Shopify Sidekick look up your Weaverse projects, pages, and SEO details directly.
</Update>

<Update label="2026.6.19" description="June 19, 2026" tags={["Studio"]}>
  ### New Features

  - **Copy and paste styling between elements**: Copy the look of one element and paste it onto another, so you can reuse a style across your page without redoing every setting.

  ### Improvements

  - **Clearer plan options**: The "Change Plan" button is now greyed out on plans below your current one, so it's obvious which plans are upgrades.
</Update>

<Update label="2026.6.18" description="June 18, 2026" tags={["Studio"]}>
  ### Improvements

  - **Billing now runs through Shopify**: Your subscription and usage are now billed directly through Shopify. Your existing plan and usage carry over automatically — there's nothing you need to do.
</Update>

<Update label="2026.6.15" description="June 15, 2026" tags={["Studio", "API", "Marketing"]}>
  ### New Features

  - **Search in the URL picker**: The URL picker now lets you search across both your Weaverse pages and your Shopify store pages, so you can find and link to a destination without scrolling.
  - **Set image widths in rich text**: Rich-text images now come with handy width presets, and your images stay intact when you pull content through the Content API.
  - **Hand your setup to an AI assistant**: The local setup step now includes a ready-to-copy prompt, so you can hand your project setup to an AI coding assistant in a single paste.
  - **Confirm before permanent deletes**: Actions that can't be undone now ask you to confirm first, so it's harder to delete something by accident.

  ### Improvements

  - **Know why a preview isn't loading**: The editor now tells the difference between a page that simply has no Weaverse content yet and a preview that's genuinely disconnected, so the message matches what's actually going on.
  - **Easier-to-read delete buttons**: Buttons for destructive actions now use a bolder red that's easier to read.

  ### Bug Fixes

  - **Blog table of contents is back**: Longer articles show their table of contents again.
  - **Pages resolve more reliably**: Fixed a handful of cases where the wrong page could show up — publishing now stays on the page you're viewing, and pages match up to the right content more consistently.
  - **Correct language in page URLs**: The editor and your live storefront now use the right language prefix in page links.
</Update>


<Update label="2026.6.11" description="June 11, 2026" tags={["API"]}>
  ### Bug Fixes

  - **Content API Custom Page Handles**: The page detail endpoint now supports custom page handles that contain `/`. If [List Pages](/content-api/list-pages) returns `type: "CUSTOM"` and `handle: "pages/gift-shop"`, fetch it with `GET /api/v1/content/projects/:projectId/pages/CUSTOM/pages/gift-shop`.
</Update>
...
<Update label="5.0.0" description="May 27, 2025" tags={["SDK"]}>
  ...
  This major release migrates Weaverse Hydrogen from Remix to React Router v7, aligning with Shopify Hydrogen's framework migration. See the current [Hydrogen updates](https://hydrogen.shopify.dev/updates).
