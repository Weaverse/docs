...
<Note>
  Current Pilot uses React 19, so new components can accept `ref` as a normal prop. Pass it to the root element and spread the remaining Weaverse props. Existing `forwardRef` components remain supported. See the [Weaverse Component guide](/development-guide/weaverse-component) for details.
</Note>
