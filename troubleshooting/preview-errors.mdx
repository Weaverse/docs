...
```javascript
// Check whether the Weaverse storefront runtime booted in the preview.
// `window.__weaverse` is the runtime instance exposed by @weaverse/hydrogen.
window.__weaverse?.sdkVersion // e.g. "5.19.1" when the runtime is mounted
window.__weaverse?.isDesignMode // true when the page is rendered inside Studio

// Multi-project storefronts expose one runtime per project id:
window.__weaverses // Record<projectId, WeaverseHydrogen>
```

<Note>
Check the storefront runtime (`window.__weaverse`), not the Studio bridge. The bridge object is an internal implementation detail and does not expose a stable `version` field.
</Note>
...
2. **Community Support**
   ...
   - Search [GitHub Discussions](https://github.com/orgs/Weaverse/discussions)
...
3. **Use recommended Node.js version**
   - Node.js 22.12 or newer for the current Pilot template
