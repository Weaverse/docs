...
3.  Choose the resource group you want, such as **Products**, then click **Create template**. The selected group determines the template type.
4.  Enter the **Template name** (e.g., `Product - Preorder`, `Collection - Lookbook Grid`) and select an existing template under **Based on**, if available.
    ![Create page template dialog with product assignments](/images/custom-templates/create-page-template-dialog.png)
5.  Optionally select resources under **Assignments**, or leave it empty to create a draft.
6.  Click **Create template** or **Create draft** as shown. The new template starts with the selected base template's content, rather than necessarily starting blank.
...
Your loader should look similar to the standard implementation, focusing on fetching Shopify data and then calling `loadPage`. Pilot's product route already does this — it lives at `app/routes/products/product.tsx` and is registered in `app/routes.ts` as `route(":productHandle", "routes/products/product.tsx")` inside the `products` prefix.

```tsx
// app/routes/products/product.tsx
// Registered in app/routes.ts — a file under app/routes/ is not a route by itself.
...
export async function loader({ params, context }: LoaderFunctionArgs) {
  ...
  // 1. Fetch product data from Shopify. `storefront.query` throws on
  //    transport/GraphQL errors, so wrap it in try/catch if you need to.
  ...
  // 3. `loadPage()` returns `WeaverseLoaderData | null` — on failure it
  //    resolves to `null` rather than throwing, so a try/catch around it
  //    would never fire. Check the result instead; Pilot's
  //    `validateWeaverseData` helper throws a 404 for a missing page.
  validateWeaverseData(weaverseData);

  // 4. Return all necessary data (a plain object — no `json()`)
