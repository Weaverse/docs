...
The Judge.me integration reads the current product from the React Router loader, then fetches reviews from a resource route. Current Pilot keeps review data out of the product loader and loads it on demand:
...
interface ReviewIndexProps extends ComponentProps<"div"> {}

function ReviewIndex({ ref, children, ...props }: ReviewIndexProps) {
  const { product } = useLoaderData<typeof productRouteLoader>();
  const { setData } = useJudgemeStore();
  ...
  useEffect(() => {
    ...
    fetch(reviewsAPI)
      ...
      .then((reviews: JudgemeReviewsData) => setData(reviews));
  }, [product?.handle, reviewsAPI, setData]);
...
1. Reuses the product already loaded by the Hydrogen route
2. Defers the third-party review request until the component renders
3. Keeps review pagination and caching independent from the product loader
