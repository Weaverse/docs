   ...
   export const loader = async ({ weaverse, data }: ComponentLoaderArgs<ApiWidgetData>) => {
     ...
     try {
       return await fetchWithCache(
         ...
         {
           headers: {
             'Authorization': ['Bearer', env.THIRD_PARTY_API_KEY].join(' '),
...
export const loader = async ({ weaverse, data }: ComponentLoaderArgs<WidgetData>) => {
  ...
  return await fetchWithCache('https://api.third-party.com/data', {
    headers: {
      'Authorization': ['Bearer', env.API_KEY].join(' ')
    }
  })
}
```
...
<Card title="Integrate Judge.me Reviews with Shopify Hydrogen" icon="star" href="/hydrogen-integrations/judgeme-reviews">
...
The Ali Reviews example below demonstrates the same general component-loader pattern.
  ...
  let res = await weaverse
    .fetchWithCache<AliReviewsAPIPayload>(
      "https://widget-hub-api.alireviews.io/api/public/reviews",
      {
        method: "GET",
        headers: {
          Authorization: ["Bearer", aliReviewsApiKey].join(" "),
          "Content-Type": "application/json",
        },
      },
    )
