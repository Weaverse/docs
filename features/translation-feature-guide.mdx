...
| Input type | Value it stores | Studio control |
| --- | --- | --- |
| `translation-key` | A translatable string | Text input |
| `translation-image` | An image URL, translatable per language | Media picker |

<Note>
  These input types require `@weaverse/hydrogen` `5.20.1` or higher (which ships `@weaverse/schema` `0.14.0`). On earlier versions TypeScript rejects them with *"Type `'translation-image'` is not assignable to type `InputType`."*
</Note>

### The one rule that makes these different
...
```ts
// Every other input: `name` is a prop
{ type: 'text', name: 'heading' }        // → props.heading

// Translation inputs: `name` is a content key
{ type: 'translation-key', name: 'cart.general.title' }   // → t('cart.general.title')
```

The consequence is important: values are stored **per locale** in the translation store, not in theme settings data. They never show up in your theme settings object or in component props — `t()` is the only way to read them back.

### Declaring them in the theme schema

Add them to the theme schema's `settings` array, which is a list of `{ group, inputs }` groups:
      ...
      inputs: [
        {
          ...
          name: 'cart.general.title',
          label: 'Cart title',
          helpText: 'Shown at the top of the cart drawer.',
        },
        {
          type: 'translation-key',
          name: 'cart.general.checkout',
...
Each `name` must match a key that already exists in your `i18n.staticContent` JSON — that file stays the source of truth for the default language.

### Reading the values back

Exactly as you would any other theme string — with `t()`:
...
export function CartHeader() {
  ...
  return (
    <div>
      ...
      <button>{t("cart.general.checkout")}</button>
    </div>
  );
}
```
  ...
  inputs: [
    {
      ...
      label: 'Promo banner',
      helpText: 'Merchants can upload a different image per language.',
    },
    {
      type: 'translation-key',
      name: 'promo.banner.alt',
...
Both read back through the same `t()` lookup — the image one returns a URL string:

```tsx
export function PromoBanner() {
  const { t } = useTranslation();

  return <img src={t("promo.banner.image")} alt={t("promo.banner.alt")} />;
}
```

<Warning>
  Use these two input types in the **theme** schema's `settings` only. They are not meant for component/section schemas — a section input's `name` must be a real component prop, and these deliberately are not.
</Warning>

<Tip>
  Reach for these when a string is worth surfacing to merchants without opening the `Translation Manager` — a promo headline, a shipping notice, a seasonal banner. For the long tail of UI strings, leave them in `i18n.staticContent` and translate them in the manager's `Theme` scope.
</Tip>
...
In Studio, click the **Advanced features** icon in the topbar to open Translation Manager:

**Advanced features → Translation → Translation Manager**

![Advanced features icon in the Weaverse Studio topbar](../images/translation/advanced-features-topbar.png)
...
7. **`Type '"translation-image"' is not assignable to type 'InputType'`:** your installed `@weaverse/schema` predates these input types. Upgrade to `@weaverse/hydrogen@5.20.1` or higher, which pins `@weaverse/schema@0.14.0`. Upgrading `@weaverse/schema` alone is not enough if `@weaverse/hydrogen` still pins an older version — check with `npm ls @weaverse/schema`.
8. **A `translation-key` input shows nothing in Studio:** its `name` must match a key that exists in `i18n.staticContent`. Unlike other inputs, `name` is a content key, not a component prop — a typo silently yields an empty field.
