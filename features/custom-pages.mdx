...
In multilingual storefronts, the first segment of the URL may be a locale (e.g., `/en-us`) or a custom page handle. With the `:locale?` prefix, an unrecognized first segment falls through to the catch-all, so the catch-all resolves it as a custom page.
...
Pilot's `app/routes/catch-all.tsx` loads the custom page with `type: "CUSTOM"` — the handle is resolved from the request URL, so no `handle` argument is needed:
...
Root-level custom pages need no extra route — the catch-all already covers them. Register a dedicated route in `app/routes.ts` only when you want a different URL shape (e.g. nesting every campaign under `/promo/...`), and load it with `type: "CUSTOM"` too.
