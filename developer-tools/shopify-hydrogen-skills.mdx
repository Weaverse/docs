...
<CardGroup cols={2}>
  <Card title="setup-weaverse-project" icon="rocket">
    The front door — take a project from nothing to a running, connected storefront. Scaffolds with the Weaverse CLI, generates `SESSION_SECRET`, boots a live demo-store preview before credentials, then makes it yours: env, GitHub repo, dev server, preview connection. Load when starting a brand-new project or running a theme locally for the first time.
  </Card>
...
- **MCP** provides real-time doc search plus account access: read projects, pages, theme settings, and locales — and edit page content, theme settings, and project names
